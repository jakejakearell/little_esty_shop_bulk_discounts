...
describe 'Admin Invoices Index Page' do
  before :each do
    ...
    @m2 = Merchant.create!(name: 'Merchant 1')

    @discount = @m1.discounts.create!(quantity_threshold: 90, percentage_discount: 0.5 )
    @discount_2 = @m2.discounts.create!(quantity_threshold: 10, percentage_discount: 0.5 )
    ...
    @item_3 = Item.create!(name: 'rest', description: 'dont test me', unit_price: 12, merchant_id: @m2.id)

    @ii_1 = InvoiceItem.create!(invoice_id: @i1.id, item_id: @item_1.id, quantity: 12, unit_price: 2, status: 0)
    @ii_2 = InvoiceItem.create!(invoice_id: @i1.id, item_id: @item_2.id, quantity: 6, unit_price: 1, status: 1)
    @ii_3 = InvoiceItem.create!(invoice_id: @i2.id, item_id: @item_2.id, quantity: 87, unit_price: 12, status: 2)
    @ii_4 = InvoiceItem.create!(invoice_id: @i3.id, item_id: @item_3.id, quantity: 20, unit_price: 12, status: 0)
  ...
  it 'will include discounts when displaying total revenue' do
    visit admin_invoice_path(@i3)

    expect(page).to have_content("Total Revenue: $120.0")
